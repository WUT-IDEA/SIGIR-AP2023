...
def main():
    ...
    bundle_train_data, bundle_test_data, item_data, assist_data, category_data = \
        dataset.get_dataset(CONFIG['path'], CONFIG['dataset_name'], task=CONFIG['task'])

    train_loader = DataLoader(dataset=bundle_train_data, batch_size=128, shuffle=True,
    ...
    ic_graph = category_data.ground_truth_i_c
    ...
    for lr, decay, message_dropout, node_dropout \
        ...
        if CONFIG['model'] == 'BGCN':
            graph = [ub_graph, ui_graph, bi_graph, ic_graph]

            info = BGCN_Info(64, decay, message_dropout, node_dropout, num_layers=1)

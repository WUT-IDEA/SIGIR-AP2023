...
class BasicDataset(Dataset):
    ...
    def __init__(self, path, name, task, neg_sample):
        ...
        self.num_users, self.num_bundles, self.num_items, self.num_categories = self.__load_data_size()
    ...
    def __load_data_size(self):
        if self.name == 'MealRec-77':
            return [1575, 3817, 7280, 3]
        elif self.name == 'MealRec-17':
    ...
    def load_B_I_affiliation(self):
        with open(os.path.join(self.path, self.name, 'bundle_item.txt'), 'r') as f:
            return list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))
    def load_I_C_affiliation(self):
        with open(os.path.join(self.path, self.name, 'item_category.txt'), 'r') as f:
            return list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))
...
class CategoryDataset(BasicDataset):
    def __init__(self, path, name):
        super().__init__(path, name, None, None)
        # self.I_C_pairs = list(set(self.load_I_C_affiliation()))
        self.I_C_pairs = self.load_I_C_affiliation()
        indice = np.array(self.I_C_pairs, dtype=np.int32)
        values = np.ones(len(self.I_C_pairs), dtype=np.float32)
        self.ground_truth_i_c = sp.coo_matrix(
            (values, (indice[:, 0], indice[:, 1])), shape=(self.num_items, self.num_categories)).tocsr()

        print_statistics(self.ground_truth_i_c, 'I-C statistics')
...
def get_dataset(path, name, task='tune', seed=123):
    ...
    return bundle_train_data, bundle_test_data, item_data, assist_data, category_data
